...
public class TC_TableModel extends AbstractTableModel
{

	private ArrayList<TC_Variable> vars;
	private String[] columnNames = {"Variable name", "Variable type", "Class", "Static", "Verified"};

	public TC_TableModel()
	{
		vars = new ArrayList<TC_Variable>();
	}
	
	public void addVariable(String var)
	{
		for(TC_Variable name : vars) 
			if(var.substring(0, var.indexOf(", ")).equals(name.getVarName()))
				return;

		String[] temp = var.split(", ");
		vars.add(new TC_Variable(temp[0], temp[1], temp[2], Boolean.parseBoolean(temp[3])));
	...
	{
		TC_Variable currentVar = vars.get(row);
